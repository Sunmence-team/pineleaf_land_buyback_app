...
export default function SubmitDocumentsScreen() {
  ...
  const handleSubmit = () => {
    if (!allDocumentsSubmitted) {
      showErrorToast(
        "Incomplete documents: Please submit all required documents",
      );
      return;
    }

    showSuccessToast(
      "Documents Submitted: Your documents are now under verification",
    );
    // Example navigation
      ...
      >
        ...
        <View className="bg-[#F7F7F7] rounded-[32px] p-4">
          ...
          <Text className="text-[16px] text-black leading-10 mb-8">
            Documents must be submitted and verified before payout.
          </Text>
          ...
          <View className="border border-[#D8D8D8] rounded-[24px] p-5">
            ...
            {documents.map((item) => (
              ...
              >
                {/* Checkbox */}
                <View
                  className={`w-12 h-12 rounded-xl border items-center justify-center mt-1 ${
                    item.submitted
          ...
          <View className="border border-[#D8D8D8] rounded-[24px] p-5 mt-6">
            <Text className="text-[#245B36] text-[16px] leading-8 mb-6">
              Your selected office submission will be verified by the company
            ...
            <View className="flex-row items-center gap-2 mb-3">
              <Ionicons name="location-outline" size={18} color="#245B36" />
            ...
            <View className="mt-6">
              <View className="flex-row justify-between mb-2">
                <Text className="text-[#6B6B6B]">Submission Progress</Text>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-[#F5F5F2] px-4 py-6">
        <TouchableOpacity
          ...
          className={`rounded-[20px] py-5 items-center ${
            allDocumentsSubmitted ? "bg-primary" : "bg-[#9CA3AF]"
          }`}
        >
